...
test("server-renders the Crema Lab brand and commercial website", async () => {
  ...
  assert.match(html, /01 — Về Crema Lab/);
  assert.match(html, /02 — Kiến thức/);
  assert.match(html, /03 — Giảng viên/);
  assert.match(html, /04 — Khóa học/);
  assert.match(html, /05 — Học phí/);
  assert.match(html, /06 — Lịch học/);
  assert.match(html, /07 — Tư vấn/);
...
test("uses a contemporary editorial system for the public Crema Lab site", async () => {
  ...
  assert.doesNotMatch(styles, /border-radius:2[0-9]px/);
});
